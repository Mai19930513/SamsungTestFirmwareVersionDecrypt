...
if __name__ == '__main__':
    ...
    now=datetime.utcnow().replace(tzinfo=timezone.utc).astimezone(SHA_TZ).strftime('%Y-%m-%d %H:%M')
    decDicts = {"update":now}
    FilePath = './firmware.json'
    ...
    for model in modelDic:
        ...
        decDicts.update(DecryptionFirmware(md5list, model))
    file.write(json.dumps(decDicts))
    file.close()
